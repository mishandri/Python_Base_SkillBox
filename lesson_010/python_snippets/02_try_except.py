...
try:
    ...
    print(f'вот что пошло не так - {exc}, но мы еще на плаву')
# имя переменной для указания на объект исключения произвольное, но принято использовать exc
...
try:
    ...
    print(f'вот что пошло не так - {exc} параметры {exc.args}, но мы еще на плаву')
# вообще, состав объекта исключения зависит от ошибки, надо смотреть документацию.
...
f = None
try:
    # Тут мы пишем код, как будто ошибок не будет... наивные...
    f = open('myfile.txt')
    s = f.readline()
    i = int(s.strip())
